...
if(FALSE){
  ...
  BiocManager::install(c("pd.clariom.d.human","affy","affycoretools","GEOquery","org.Bt.eg.db","AnnotationFuncs","arrayQualityMetrics"))
...
# logFC -+ 
  # t(0.975, efit$df.residual + efit$df.prior)*efit$stdev.unscaled[,2]*sqrt(efit$s2.post)

# 95% CI
# deg_vsDAFinside$ci <-  sqrt(efit$s2.post) * efit$stdev.unscaled[,2]*1.96
